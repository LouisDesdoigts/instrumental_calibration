...
The optimisation is performed for a total of 100 epochs, with each gradient calculation taking $\sim$ 2.75 seconds to evaluate on an Apple M1 Max CPU, leading to a total optimisation time of $\sim$ 4.5 minutes.
...
are generated by harnessing the differentiablility of the model by calculating the covaraince matrix under the Laplace approximation, defined as the negative matrix inverse of the hessian of the likelihood function evaluated at the maximum likelihood estimate, ie the final recovered values. These indicate a poor recovery, however they are only marginalised over positions, fluxes and aberration, not the \ac{prf} which is the dominant noise source as its high dimensionality makes marginalisation computationally intractable. These parameters are recovered to the expected distribution when performed without modelling the \ac{prf}. Figure~\ref{fig:aberrations} shows the recovery of the optical aberrations, with a final residual aberration of 
% \textcolor{red}{showyourwork variable}
