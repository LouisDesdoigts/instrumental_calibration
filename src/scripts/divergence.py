...
aperture = tel.CreateWavefront.diameter
wf_npix = tel.CreateWavefront.npixels
...
# Update detecotr parameters
tel = tel.set(["AngularMFT.npixels_out", "AngularMFT.pixel_scale_out"], [det_npix, det_pixsize])
...
tel = tel.set("detector", detector)

# Create Source object
source = dl.PointSource(np.zeros(2), 1e3, wavelengths=wavels)
tel = tel.set('scene.sources', {source.name: source})
...
"""Define functions to batch all the optical systems"""
...
def initialise_models(model):
    # Perturb the positions by a small amount
    ...
    model = model.set(zernikes, np.zeros(len(tel.get(zernikes))))
